...
func (view *View) Success(w http.ResponseWriter, r *http.Request) {
	err := renderTemplate(w, "np-success.tpl", nil)
	if err != nil {
		log.Println("Can't render about page ", err)
	}
}

func (view *View) PasswordReset(w http.ResponseWriter, r *http.Request) {
	err := renderTemplate(w, "password-reset.tpl", nil)
	if err != nil {
		log.Println("Can't render about page ", err)
	}
}

func (view *View) UNP(w http.ResponseWriter, r *http.Request) {
	err := renderTemplate(w, "user-new-password.tpl", nil)
	if err != nil {
		log.Println("Can't render about page ", err)
	}
}
...
func (view *View) LinkCreation(w http.ResponseWriter, r *http.Request) {
	err := r.ParseForm()
	if err != nil {
		log.Println("Can't parse form ", err)
		return
	...
	newId := rand.Intn(1000000-100000+1) + 100000
	userMail := r.Form.Get("email")

	newEntry := models.Pwdreset{
		ID:    newId,
		Email: userMail,
	}

	view.DB.AddEntry(newEntry)

	htmlMessage := fmt.Sprintf(`
	<a href="http://localhost:8080/user-new-password?userid=%d">Click here to reset your account password!</a><br>`, newId)
	...
	http.Redirect(w, r, "np-success", http.StatusSeeOther)
}

func (view *View) ResetPasswdFromLink(w http.ResponseWriter, r *http.Request) {

	id := r.URL.Query().Get("userid")
	log.Println("id:", id)

	uid, _ := strconv.Atoi(id)
	user, err := view.DB.GetResetPasswdUser(uid)
	...
	if parseErr != nil {
		log.Println("Can't parse form ", parseErr)
		return
	}

	form := forms.New(r.PostForm)
	form.CheckRequiredFields("password")

	if form.IsNotValid() {
		renderTemplate(w, "user-new-password.tpl", &TemplateData{
			Form: form,
		})
	...
	rawPasswd := r.Form.Get("password")
	encryptedPassword, errPasswd := encryptPassword(rawPasswd)
	log.Println("password: ", rawPasswd)

	if errPasswd != nil {
		log.Println("error passwd", errPasswd)
		return
	}

	updateErr := view.DB.UpdateUserPassword(user.Email, encryptedPassword)
	if updateErr != nil {
		log.Println("Can't update user", updateErr)
		return
	}
