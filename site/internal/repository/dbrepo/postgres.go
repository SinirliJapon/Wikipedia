...
func (pg *postgresDBRepo) UpdateUserPassword(email string, password string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func (pg *postgresDBRepo) AddEntry(logg models.Pwdreset) (int, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	stmt := `insert into pwdreset (id, 
		email) 
		values ($1, $2)
		`

	row := pg.DB.QueryRowContext(ctx, stmt, logg.ID, logg.Email)
...
func (pg *postgresDBRepo) GetResetPasswdUser(uid int) (models.Pwdreset, error) {

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	stmt := `select * from pwdreset where id = $1`

	var user models.Pwdreset

	row := pg.DB.QueryRowContext(ctx, stmt, uid)
