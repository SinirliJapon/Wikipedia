...
func CreateRoutes(view *handlers.View) http.Handler {
	...
	mux.Get("/user-new-password", view.UNP)
	mux.Post("/user-new-password", view.ResetPasswdFromLink)
	mux.Get("/np-success", view.Success)

	mux.Get("/password-reset", view.PasswordReset)
	mux.Post("/password-reset", view.LinkCreation)
